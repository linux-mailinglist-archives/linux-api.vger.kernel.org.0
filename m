...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 710C42CE3FA
	for <lists+linux-api@lfdr.de>; Fri,  4 Dec 2020 01:09:45 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2388814AbgLDAI4 (ORCPT <rfc822;lists+linux-api@lfdr.de>);
        Thu, 3 Dec 2020 19:08:56 -0500
Received: from youngberry.canonical.com ([91.189.89.112]:42644 "EHLO
        youngberry.canonical.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726619AbgLDAI4 (ORCPT
        <rfc822;linux-api@vger.kernel.org>); Thu, 3 Dec 2020 19:08:56 -0500
Received: from ip5f5af0a0.dynamic.kabel-deutschland.de ([95.90.240.160] helo=wittgenstein.fritz.box)
        by youngberry.canonical.com with esmtpsa (TLS1.2:ECDHE_RSA_AES_128_GCM_SHA256:128)
        (Exim 4.86_2)
        (envelope-from <christian.brauner@ubuntu.com>)
        id 1kkyXn-0007ka-1l; Fri, 04 Dec 2020 00:01:51 +0000
...
Subject: [PATCH v4 20/40] namei: prepare for idmapped mounts
Date:   Fri,  4 Dec 2020 00:57:16 +0100
Message-Id: <20201203235736.3528991-21-christian.brauner@ubuntu.com>
...
index 453920b493b0..6cc655310bd9 100644
--- a/fs/namei.c
+++ b/fs/namei.c
@@ -2870,10 +2870,26 @@ void unlock_rename(struct dentry *p1, struct dentry *p2)
 }
...
@@ -3349,7 +3365,23 @@ static int do_open(struct nameidata *nd,
 	return error;
 }
...
@@ -3357,7 +3389,7 @@ struct dentry *vfs_tmpfile(struct dentry *dentry, umode_t mode, int open_flag)
 	int error;
...
+	error = inode_permission(mnt_userns, dir, MAY_WRITE | MAY_EXEC);
 	if (error)
 		goto out_err;
 	error = -EOPNOTSUPP;
@@ -3392,6 +3424,7 @@ static int do_tmpfile(struct nameidata *nd, unsigned flags,
...
@@ -3400,7 +3433,8 @@ static int do_tmpfile(struct nameidata *nd, unsigned flags,
...
+	child = vfs_tmpfile(mnt_userns, path.dentry, op->mode, op->open_flag);
 	...
 	if (IS_ERR(child))
 		goto out2;
@@ -3612,10 +3646,27 @@ inline struct dentry *user_path_create(int dfd, const char __user *pathname,
 }
...
+	int error = may_create(mnt_userns, dir, dentry);
 	...
 	if (error)
 		return error;
@@ -3662,6 +3713,7 @@ static int may_mknod(umode_t mode)
...
+	struct user_namespace *mnt_userns;
 	...
 	struct path path;
 	int error;
@@ -3680,18 +3732,22 @@ static long do_mknodat(int dfd, const char __user *filename, umode_t mode,
 	error = security_path_mknod(&path, dentry, mode, dev);
 	if (error)
 		goto out;
+
...
+					  dentry, mode, 0);
 ...
 out:
@@ -3714,9 +3770,25 @@ SYSCALL_DEFINE3(mknod, const char __user *, filename, umode_t, mode, unsigned, d
...
+	int error = may_create(mnt_userns, dir, dentry);
 	...
 	if (error)
@@ -3755,8 +3827,11 @@ static long do_mkdirat(int dfd, const char __user *pathname, umode_t mode)
...
@@ -3775,9 +3850,24 @@ SYSCALL_DEFINE2(mkdir, const char __user *, pathname, umode_t, mode)
...
+	int error = may_delete(mnt_userns, dir, dentry, 1);
 	...
 	if (error)
 		return error;
@@ -3817,6 +3907,7 @@ EXPORT_SYMBOL(vfs_rmdir);
...
+	struct user_namespace *mnt_userns;
 	...
 	struct dentry *dentry;
 	struct path path;
@@ -3857,7 +3948,8 @@ long do_rmdir(int dfd, struct filename *name)
...
+	error = vfs_rmdir(mnt_userns, path.dentry->d_inode, dentry);
 exit3:
 	dput(dentry);
 exit2:
@@ -3880,6 +3972,7 @@ SYSCALL_DEFINE1(rmdir, const char __user *, pathname)
...
@@ -3895,11 +3988,18 @@ SYSCALL_DEFINE1(rmdir, const char __user *, pathname)
...
+	int error = may_delete(mnt_userns, dir, dentry, 0);
 	...
 	if (error)
 		return error;
@@ -3970,6 +4070,8 @@ long do_unlinkat(int dfd, struct filename *name)
...
+
 		...
 		if (last.name[last.len])
 			goto slashes;
@@ -3980,7 +4082,8 @@ long do_unlinkat(int dfd, struct filename *name)
...
@@ -4029,9 +4132,25 @@ SYSCALL_DEFINE1(unlink, const char __user *, pathname)
...
+	int error = may_create(mnt_userns, dir, dentry);
 	...
 	if (error)
 		return error;
@@ -4069,8 +4188,13 @@ static long do_symlinkat(const char __user *oldname, int newdfd,
 		goto out_putname;
...
+	}
 	...
 	if (retry_estale(error, lookup_flags)) {
 		lookup_flags |= LOOKUP_REVAL;
@@ -4095,6 +4219,7 @@ SYSCALL_DEFINE2(symlink, const char __user *, oldname, const char __user *, newn
 /**
...
@@ -4110,8 +4235,16 @@ SYSCALL_DEFINE2(symlink, const char __user *, oldname, const char __user *, newn
  * Alternatively, a caller may pass NULL for delegated_inode.  This may
  * be appropriate for callers that expect the underlying filesystem not
  * to be NFS exported.
+ *
...
+	     struct inode **delegated_inode)
 {
 	...
 	unsigned max_links = dir->i_sb->s_max_links;
@@ -4120,7 +4253,7 @@ int vfs_link(struct dentry *old_dentry, struct inode *dir, struct dentry *new_de
 	if (!inode)
 		return -ENOENT;
...
@@ -4137,7 +4270,7 @@ int vfs_link(struct dentry *old_dentry, struct inode *dir, struct dentry *new_de
...
+	if (HAS_UNMAPPED_ID(mnt_userns, inode))
 	...
 	if (!dir->i_op->link)
 		return -EPERM;
@@ -4184,6 +4317,7 @@ EXPORT_SYMBOL(vfs_link);
...
@@ -4225,7 +4359,9 @@ static int do_linkat(int olddfd, const char __user *oldname, int newdfd,
 	error = security_path_link(old_path.dentry, &new_path, new_dentry);
 	if (error)
 		goto out_dput;
...
+			 new_dentry, &delegated_inode);
 out_dput:
 	done_path_create(&new_path, new_dentry);
 	if (delegated_inode) {
@@ -4259,12 +4395,14 @@ SYSCALL_DEFINE2(link, const char __user *, oldname, const char __user *, newname
...
+ * @flags:		rename flags
  ...
  *
@@ -4326,19 +4464,19 @@ int vfs_rename(struct renamedata *rd)
 	if (source == target)
 		return 0;
...
+			error = may_delete(rd->new_mnt_userns, new_dir, new_dentry, new_is_dir);
 	...
 	if (error)
 		return error;
@@ -4352,12 +4490,12 @@ int vfs_rename(struct renamedata *rd)
 	 */
 	if (new_dir != old_dir) {
 		if (is_dir) {
...
@@ -4548,8 +4686,10 @@ static int do_renameat2(int olddfd, const char __user *oldname, int newdfd,
...
index 3d7a8cd61098..00d181829fe0 100644
--- a/fs/nfsd/vfs.c
+++ b/fs/nfsd/vfs.c
...
index 73da8710b0f0..5556e85552c1 100644
...
index 6f0fb7d8d2c4..c88dafc4152f 100644
--- a/include/linux/fs.h
+++ b/include/linux/fs.h
@@ -1769,17 +1769,20 @@ extern bool inode_owner_or_capable(struct user_namespace *mnt_userns,
 /*
...
+	struct user_namespace *new_mnt_userns;
 	...
 	struct dentry *new_dentry;
 	struct inode **delegated_inode;
@@ -1788,12 +1791,14 @@ struct renamedata {
