...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 01E7D2F3EE4
	for <lists+linux-api@lfdr.de>; Wed, 13 Jan 2021 01:45:50 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S2394506AbhALWNm (ORCPT <rfc822;lists+linux-api@lfdr.de>);
        Tue, 12 Jan 2021 17:13:42 -0500
Received: from youngberry.canonical.com ([91.189.89.112]:44368 "EHLO
        youngberry.canonical.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S2391550AbhALWNk (ORCPT
        <rfc822;linux-api@vger.kernel.org>); Tue, 12 Jan 2021 17:13:40 -0500
Received: from ip5f5af0a0.dynamic.kabel-deutschland.de ([95.90.240.160] helo=wittgenstein.fritz.box)
        by youngberry.canonical.com with esmtpsa (TLS1.2:ECDHE_RSA_AES_128_GCM_SHA256:128)
        (Exim 4.86_2)
        (envelope-from <christian.brauner@ubuntu.com>)
        id 1kzRlq-0003bd-Qi; Tue, 12 Jan 2021 22:04:12 +0000
...
Subject: [PATCH v5 32/42] fs: make helpers idmap mount aware
Date:   Tue, 12 Jan 2021 23:01:14 +0100
Message-Id: <20210112220124.837960-33-christian.brauner@ubuntu.com>
X-Mailer: git-send-email 2.30.0
In-Reply-To: <20210112220124.837960-1-christian.brauner@ubuntu.com>
References: <20210112220124.837960-1-christian.brauner@ubuntu.com>
MIME-Version: 1.0
X-Patch-Hashes: v=1; h=sha256; i=0K5N8J9d2sWh2vlZe5yL8LXLhBBwbwnMB7i8zkpl8kY=; m=OXNDuiikklYuOaN3GWp7jTqVh/ffe/lK6/ZwymbPcyc=; p=n9vxMpurD5aGkhxQUYAy6k56fTdx/28sVQZprkVYKZs=; g=a8147b925634f09c737c45c178100741554871b1
X-Patch-Sig: m=pgp; i=christian.brauner@ubuntu.com; s=0x0x91C61BC06578DCA2; b=iHUEABYKAB0WIQRAhzRXHqcMeLMyaSiRxhvAZXjcogUCX/4YuAAKCRCRxhvAZXjcouC1APwJTYJ yCJmRD92RjfMwcUPlBmg3mPmsUdNigZJ3c0Pj3wD+JtcjPbcHjHX2b1td4ThIv/xuHUx5oKX1sPzZ xclQpA4=
...
---
 ...
 183 files changed, 1056 insertions(+), 759 deletions(-)
...
index 984f28315d2a..5bd6981ff5d3 100644
...
index c8045c9d51d9..766c226472d5 100644
--- a/fs/btrfs/inode.c
+++ b/fs/btrfs/inode.c
...
@@ -6351,8 +6352,8 @@ static int btrfs_add_nondir(struct btrfs_trans_handle *trans,
 	return err;
 }
...
@@ -6415,8 +6416,8 @@ static int btrfs_mknod(struct inode *dir, struct dentry *dentry,
 	return err;
 }
...
+			struct dentry *dentry, umode_t mode, bool excl)
 {
 	struct btrfs_fs_info *fs_info = btrfs_sb(dir->i_sb);
 	struct btrfs_trans_handle *trans;
@@ -6560,7 +6561,8 @@ static int btrfs_link(struct dentry *old_dentry, struct inode *dir,
 	return err;
 }
...
@@ -8815,7 +8817,8 @@ int __init btrfs_init_cachep(void)
 	return -ENOMEM;
 }
...
+			 const struct path *path, struct kstat *stat,
 ...
 {
 	u64 delalloc_bytes;
@@ -9332,9 +9335,9 @@ static int btrfs_rename(struct inode *old_dir, struct dentry *old_dentry,
 	return ret;
 }
...
+			 struct dentry *new_dentry, unsigned int flags)
 {
 	if (flags & ~(RENAME_NOREPLACE | RENAME_EXCHANGE | RENAME_WHITEOUT))
 		return -EINVAL;
@@ -9516,8 +9519,8 @@ int btrfs_start_delalloc_roots(struct btrfs_fs_info *fs_info, u64 nr,
 	return ret;
 }
...
+			 struct dentry *dentry, const char *symname)
 {
 	struct btrfs_fs_info *fs_info = btrfs_sb(dir->i_sb);
 	struct btrfs_trans_handle *trans;
@@ -9851,7 +9854,8 @@ static int btrfs_set_page_dirty(struct page *page)
...
@@ -9866,7 +9870,8 @@ static int btrfs_permission(struct inode *inode, int mask)
...
index 179a2bb88538..a19877c5e909 100644
--- a/fs/ceph/inode.c
+++ b/fs/ceph/inode.c
...
@@ -2321,7 +2322,7 @@ int __ceph_do_getattr(struct inode *inode, struct page *locked_page,
...
@@ -2368,8 +2369,8 @@ static int statx_to_caps(u32 want, umode_t mode)
...
index ce14e6f8adb6..0de75184e585 100644
--- a/fs/cifs/cifsfs.c
+++ b/fs/cifs/cifsfs.c
@@ -305,7 +305,7 @@ static long cifs_fallocate(struct file *file, int mode, loff_t off, loff_t len)
 	return -EOPNOTSUPP;
 }
...
index 2307bb0f6147..5936f4042108 100644
--- a/fs/cifs/cifsfs.h
+++ b/fs/cifs/cifsfs.h
...
index 61ae826d5a16..c3223e393542 100644
--- a/fs/exec.c
+++ b/fs/exec.c
...
index ea980f1e2e99..e7f08c83e4d9 100644
--- a/fs/ext2/namei.c
+++ b/fs/ext2/namei.c
...
index 2866d249f3d2..4846168249c2 100644
--- a/fs/ext4/ext4.h
+++ b/fs/ext4/ext4.h
@@ -2877,11 +2877,13 @@ extern struct inode *__ext4_iget(struct super_block *sb, unsigned long ino,
...
index 69caa7b939d3..6e11e2d9d4a2 100644
--- a/fs/ext4/inode.c
+++ b/fs/ext4/inode.c
@@ -5315,7 +5315,8 @@ static void ext4_wait_for_tail_page_commit(struct inode *inode)
  *
...
+		 struct iattr *attr)
 {
 	struct inode *inode = d_inode(dentry);
 	int error, rc = 0;
@@ -5531,8 +5532,8 @@ int ext4_setattr(struct dentry *dentry, struct iattr *attr)
 	return error;
 }
...
@@ -5571,13 +5572,14 @@ int ext4_getattr(const struct path *path, struct kstat *stat,
 	return 0;
 }
...
index b17a082b7db1..e0eb5e37f65a 100644
--- a/fs/ext4/namei.c
+++ b/fs/ext4/namei.c
...
+		      struct dentry *dentry, umode_t mode)
 {
 	handle_t *handle;
 	struct inode *inode;
@@ -3286,7 +3288,7 @@ static int ext4_unlink(struct inode *dir, struct dentry *dentry)
 	return retval;
 }
...
+static int ext4_symlink(struct user_namespace *mnt_userns, struct inode *dir,
 ...
 {
 	handle_t *handle;
@@ -4078,7 +4080,8 @@ static int ext4_cross_rename(struct inode *old_dir, struct dentry *old_dentry,
 	return retval;
 }
...
index 144e434834b3..0af485ba72d8 100644
--- a/fs/f2fs/acl.c
+++ b/fs/f2fs/acl.c
...
index 5020c740f068..f9b5434fb8a9 100644
--- a/fs/f2fs/file.c
+++ b/fs/f2fs/file.c
...
@@ -859,7 +859,8 @@ static void __setattr_copy(struct user_namespace *mnt_userns, struct inode *inod
...
index f7e04f533d31..80fbf1b5bb6c 100644
--- a/fs/fat/file.c
+++ b/fs/fat/file.c
...
+	err = fat_setattr(file_user_ns(file), file->f_path.dentry, &ia);
 	if (err)
 		goto out_unlock_inode;
...
index f2f252e3da7f..36961f9bedc1 100644
--- a/fs/fuse/dir.c
+++ b/fs/fuse/dir.c
...
@@ -1756,7 +1759,8 @@ int fuse_do_setattr(struct dentry *dentry, struct iattr *attr,
 	return err;
 }
...
+			struct iattr *attr)
 {
 	...
 	struct fuse_conn *fc = get_fuse_conn(inode);
@@ -1818,7 +1822,8 @@ static int fuse_setattr(struct dentry *entry, struct iattr *attr)
 	return ret;
 }
...
index 9b773b2b9d32..9ab07a0e9707 100644
--- a/fs/gfs2/inode.c
+++ b/fs/gfs2/inode.c
...
@@ -1490,7 +1493,7 @@ static int gfs2_rename(struct inode *odir, struct dentry *odentry,
 			}
 		}
 	} else {
...
@@ -1525,7 +1528,7 @@ static int gfs2_rename(struct inode *odir, struct dentry *odentry,
...
@@ -1688,12 +1691,12 @@ static int gfs2_exchange(struct inode *odir, struct dentry *odentry,
 		goto out_gunlock;
...
@@ -1747,9 +1750,9 @@ static int gfs2_exchange(struct inode *odir, struct dentry *odentry,
 	return error;
 }
...
@@ -1833,7 +1836,8 @@ static const char *gfs2_get_link(struct dentry *dentry,
  * Returns: errno
  */
...
@@ -1963,7 +1967,8 @@ static int setattr_chown(struct inode *inode, struct iattr *attr)
  * Returns: errno
  */
...
+			struct dentry *dentry, struct iattr *attr)
 {
 	struct inode *inode = d_inode(dentry);
 	struct gfs2_inode *ip = GFS2_I(inode);
@@ -2007,6 +2012,7 @@ static int gfs2_setattr(struct dentry *dentry, struct iattr *attr)
...
@@ -2021,7 +2027,8 @@ static int gfs2_setattr(struct dentry *dentry, struct iattr *attr)
  * Returns: errno
  */
...
index 7a53eed69fef..91f5b83630f2 100644
--- a/fs/kernfs/dir.c
+++ b/fs/kernfs/dir.c
@@ -1110,8 +1110,8 @@ static struct dentry *kernfs_iop_lookup(struct inode *dir,
 	return ret;
 }
...
@@ -1147,7 +1147,8 @@ static int kernfs_iop_rmdir(struct inode *dir, struct dentry *dentry)
 	return ret;
 }
...
index 016d52fda656..7c9dd5091a25 100644
--- a/fs/namei.c
+++ b/fs/namei.c
@@ -447,7 +447,7 @@ static inline int do_inode_permission(struct user_namespace *mnt_userns,
 {
...
@@ -2203,11 +2203,13 @@ static int link_path_walk(const char *name, struct nameidata *nd)
...
@@ -2255,7 +2257,7 @@ static int link_path_walk(const char *name, struct nameidata *nd)
 OK:
...
+				nd->dir_uid = i_uid_into_mnt(mnt_userns, nd->inode);
 				...
 				return 0;
@@ -2908,7 +2910,7 @@ int vfs_create(struct user_namespace *mnt_userns, struct inode *dir,
...
+	error = dir->i_op->create(mnt_userns, dir, dentry, mode, want_excl);
 	...
 	return error;
@@ -2999,7 +3001,7 @@ static int may_open(struct user_namespace *mnt_userns, const struct path *path,
 	return 0;
 }
...
@@ -3013,8 +3015,7 @@ static int handle_truncate(struct file *filp)
 	if (!error)
...
@@ -3122,6 +3123,7 @@ static struct dentry *lookup_open(struct nameidata *nd, struct file *file,
...
@@ -3169,13 +3171,14 @@ static struct dentry *lookup_open(struct nameidata *nd, struct file *file,
 	 */
...
+			create_error = may_o_create(mnt_userns, &nd->path,
 			...
 			create_error = -EROFS;
@@ -3211,8 +3214,9 @@ static struct dentry *lookup_open(struct nameidata *nd, struct file *file,
 			error = -EACCES;
 			goto out_dput;
 		}
...
@@ -3320,6 +3324,7 @@ static const char *open_last_lookups(struct nameidata *nd,
...
+	struct user_namespace *mnt_userns;
 	...
 	bool do_truncate;
 	int acc_mode;
@@ -3332,12 +3337,13 @@ static int do_open(struct nameidata *nd,
 	}
...
+		error = may_create_in_sticky(mnt_userns, nd,
 		...
 		if (unlikely(error))
 			return error;
@@ -3357,13 +3363,13 @@ static int do_open(struct nameidata *nd,
 			return error;
 		do_truncate = true;
 	}
...
+		error = handle_truncate(mnt_userns, file);
 	if (unlikely(error > 0)) {
 		WARN_ON(1);
 		error = -EINVAL;
@@ -3407,7 +3413,7 @@ struct dentry *vfs_tmpfile(struct user_namespace *mnt_userns,
...
+	error = dir->i_op->tmpfile(mnt_userns, dir, child, mode);
 	if (error)
 		goto out_err;
 	error = -ENOENT;
@@ -3450,7 +3456,7 @@ static int do_tmpfile(struct nameidata *nd, unsigned flags,
 	path.dentry = child;
...
@@ -3694,7 +3700,7 @@ int vfs_mknod(struct user_namespace *mnt_userns, struct inode *dir,
 	if (error)
 		return error;
...
+	error = dir->i_op->mknod(mnt_userns, dir, dentry, mode, dev);
 	if (!error)
 		fsnotify_create(dir, dentry);
 	return error;
@@ -3813,7 +3819,7 @@ int vfs_mkdir(struct user_namespace *mnt_userns, struct inode *dir,
...
+	error = dir->i_op->mkdir(mnt_userns, dir, dentry, mode);
 	...
 	return error;
@@ -4170,7 +4176,7 @@ int vfs_symlink(struct user_namespace *mnt_userns, struct inode *dir,
 	if (error)
 		return error;
...
+	error = dir->i_op->symlink(mnt_userns, dir, dentry, oldname);
 	if (!error)
 		fsnotify_create(dir, dentry);
 	return error;
@@ -4361,13 +4367,13 @@ static int do_linkat(int olddfd, const char __user *oldname, int newdfd,
 	error = -EXDEV;
...
-	mnt_userns = mnt_user_ns(new_path.mnt);
 ...
 out_dput:
@@ -4456,7 +4462,6 @@ SYSCALL_DEFINE2(link, const char __user *, oldname, const char __user *, newname
...
@@ -4543,8 +4548,8 @@ int vfs_rename(struct renamedata *rd)
 		if (error)
 			goto out;
 	}
...
index b840d0a91c9d..4c92337ab4dc 100644
--- a/fs/nfs/internal.h
+++ b/fs/nfs/internal.h
@@ -358,14 +358,14 @@ extern unsigned long nfs_access_cache_count(struct shrinker *shrink,
...
index fc6252ccc6d7..acad21160690 100644
...
index 5e9eb46e741a..cf40e3a4443f 100644
--- a/fs/overlayfs/overlayfs.h
+++ b/fs/overlayfs/overlayfs.h
@@ -444,10 +444,11 @@ int ovl_set_nlink_lower(struct dentry *dentry);
...
index e414dcd307b3..24404a167367 100644
--- a/fs/overlayfs/super.c
+++ b/fs/overlayfs/super.c
@@ -1021,7 +1021,7 @@ ovl_posix_acl_xattr_set(const struct xattr_handler *handler,
...
index f60b379dcdc7..d33268921eec 100644
...
index 0927247d656f..0cc6138bbff2 100644
--- a/fs/xfs/xfs_acl.c
+++ b/fs/xfs/xfs_acl.c
...
index 168b8d3f5afb..361a1e08f6c1 100644
--- a/include/linux/fs.h
+++ b/include/linux/fs.h
@@ -1925,22 +1925,22 @@ struct file_operations {
 struct inode_operations {
 	struct dentry * (*lookup) (struct inode *,struct dentry *, unsigned int);
...
+	int (*getattr) (struct user_namespace *, const struct path *, struct kstat *, u32, unsigned int);
 	...
 	int (*fiemap)(struct inode *, struct fiemap_extent_info *, u64 start,
 		      u64 len);
@@ -1948,8 +1948,8 @@ struct inode_operations {
...
@@ -3209,15 +3209,18 @@ extern int dcache_dir_open(struct inode *, struct file *);
...
index 339d5530d3a9..1b4512604dd5 100644
--- a/mm/shmem.c
+++ b/mm/shmem.c
...
+	      struct dentry *dentry, umode_t mode)
 {
 	struct inode *inode;
 	int error = -ENOSPC;
@@ -2969,20 +2973,21 @@ shmem_tmpfile(struct inode *dir, struct dentry *dentry, umode_t mode)
 	return error;
 }
...
+	return shmem_mknod(&init_user_ns, dir, dentry, mode | S_IFREG, 0);
 ...
 /*
@@ -3062,7 +3067,8 @@ static int shmem_exchange(struct inode *old_dir, struct dentry *old_dentry, stru
 	return 0;
 }
...
+			  struct dentry *old_dentry)
 {
 	...
 	int error;
@@ -3071,7 +3077,7 @@ static int shmem_whiteout(struct inode *old_dir, struct dentry *old_dentry)
 	if (!whiteout)
 		return -ENOMEM;
...
+	error = shmem_mknod(&init_user_ns, old_dir, whiteout,
 	...
 	dput(whiteout);
 	if (error)
@@ -3094,7 +3100,9 @@ static int shmem_whiteout(struct inode *old_dir, struct dentry *old_dentry)
...
@@ -3111,7 +3119,7 @@ static int shmem_rename2(struct inode *old_dir, struct dentry *old_dentry, struc
...
@@ -3135,7 +3143,8 @@ static int shmem_rename2(struct inode *old_dir, struct dentry *old_dentry, struc
 	return 0;
 }
...
index c76703c6f480..9f4dc2ea58e9 100644
--- a/net/socket.c
+++ b/net/socket.c
