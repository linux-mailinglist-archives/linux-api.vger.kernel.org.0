...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id A2319EC7C3
	for <lists+linux-api@lfdr.de>; Fri,  1 Nov 2019 18:35:20 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727947AbfKARey (ORCPT <rfc822;lists+linux-api@lfdr.de>);
        Fri, 1 Nov 2019 13:34:54 -0400
Received: from us-smtp-2.mimecast.com ([207.211.31.81]:34373 "EHLO
        us-smtp-delivery-1.mimecast.com" rhost-flags-OK-OK-OK-FAIL)
        by vger.kernel.org with ESMTP id S1729613AbfKARev (ORCPT
        <rfc822;linux-api@vger.kernel.org>); Fri, 1 Nov 2019 13:34:51 -0400
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
        s=mimecast20190719; t=1572629687;
        h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
         to:to:cc:cc:mime-version:mime-version:content-type:content-type:
         content-transfer-encoding:content-transfer-encoding:
         in-reply-to:in-reply-to:references:references;
        bh=YvdbzxCpv+fICSYb83023SgSKfIs5t/bfDI2DJc3eKg=;
        b=FckLYBTurtTVkFRlgwcjY6tlQsDRc2wpyakPvbVeHJ2wSKPMRNdYnLMMFkzARGpldP8Nua
        T3jZUWY66kdo0m3EfLhVJJVwGIW3m3Zy1pjXVT30gkgWrEFnPGa6fTbFsOpXA+gMa4YAVP
        E1jUsxBGQAW+8e6hcR9jZizfRww6URY=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-241-PAoLjEuMP7-FV82Z9sdXNA-1; Fri, 01 Nov 2019 13:34:43 -0400
Received: from smtp.corp.redhat.com (int-mx07.intmail.prod.int.phx2.redhat.com [10.5.11.22])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mimecast-mx01.redhat.com (Postfix) with ESMTPS id 97E99800A1A;
        Fri,  1 Nov 2019 17:34:41 +0000 (UTC)
Received: from warthog.procyon.org.uk (ovpn-121-40.rdu2.redhat.com [10.10.121.40])
        by smtp.corp.redhat.com (Postfix) with ESMTP id 4FB691001920;
        Fri,  1 Nov 2019 17:34:38 +0000 (UTC)
Organization: Red Hat UK Ltd. Registered Address: Red Hat UK Ltd, Amberley
 Place, 107-111 Peascod Street, Windsor, Berkshire, SI4 1TE, United
 Kingdom.
 Registered in England and Wales under Company Registration No. 3798903
Subject: [RFC PATCH 04/11] pipe: Use head and tail pointers for the ring,
 not cursor and length [ver #3]
...
Date:   Fri, 01 Nov 2019 17:34:37 +0000
Message-ID: <157262967752.13142.696874122947836210.stgit@warthog.procyon.org.uk>
...
X-MC-Unique: PAoLjEuMP7-FV82Z9sdXNA-1
...
@@ -919,6 +919,7 @@ static ssize_t port_fops_splice_write(struct pipe_inode=
_info *pipe,
 =09=09.pos =3D *ppos,
 =09=09.u.data =3D &sgl,
 =09};
+=09unsigned int occupancy;
=20
 =09/*
 =09 * Rproc_serial does not yet support splice. To support splice
@@ -929,21 +930,18 @@ static ssize_t port_fops_splice_write(struct pipe_ino=
de_info *pipe,
 =09if (is_rproc_serial(port->out_vq->vdev))
 =09=09return -EINVAL;
=20
-=09/*
-=09 * pipe->nrbufs =3D=3D 0 means there are no data to transfer,
-=09 * so this returns just 0 for no data.
-=09 */
 =09pipe_lock(pipe);
-=09if (!pipe->nrbufs) {
-=09=09ret =3D 0;
...
-=09}
=20
 =09ret =3D wait_port_writable(port, filp->f_flags & O_NONBLOCK);
 =09if (ret < 0)
 =09=09goto error_out;
=20
-=09buf =3D alloc_buf(port->portdev->vdev, 0, pipe->nrbufs);
+=09occupancy =3D pipe_occupancy(pipe->head, pipe->tail);
+=09buf =3D alloc_buf(port->portdev->vdev, 0, occupancy);
+=09=09=09
 =09if (!buf) {
 =09=09ret =3D -ENOMEM;
 =09=09goto error_out;
@@ -951,7 +949,7 @@ static ssize_t port_fops_splice_write(struct pipe_inode=
_info *pipe,
=20
 =09sgl.n =3D 0;
 =09sgl.len =3D 0;
-=09sgl.size =3D pipe->nrbufs;
+=09sgl.size =3D occupancy;
 =09sgl.sg =3D buf->sg;
 =09sg_init_table(sgl.sg, sgl.size);
 =09ret =3D __splice_from_pipe(pipe, &sd, pipe_to_sg);
...
there
+ * isn't a dead spot in the buffer, but the ring has to be a power of two =
and
+ * <=3D 2^31.
...
=20
 ...
 =09=09pipe_wait(pipe);
@@ -955,7 +972,7 @@ static int fifo_open(struct inode *inode, struct file *=
filp)
 =09=09=09}
 =09=09}
 =09=09break;
...
+
 ...
 =09/*
 =09 *  O_WRONLY
@@ -975,7 +992,7 @@ static int fifo_open(struct inode *inode, struct file *=
filp)
...
=20
 =09/*
@@ -1071,13 +1088,13 @@ static long pipe_set_size(struct pipe_inode_info *p=
...
@@ -1086,17 +1103,21 @@ static long pipe_set_size(struct pipe_inode_info *p=
ipe, unsigned long arg)
 =09}
=20
 =09/*
...
+=09bufs =3D kcalloc(nr_slots, sizeof(*bufs),
 ...
 =09if (unlikely(!bufs)) {
 =09=09ret =3D -ENOMEM;
@@ -1105,33 +1126,36 @@ static long pipe_set_size(struct pipe_inode_info *p=
ipe, unsigned long arg)
=20
 =09/*
...
=20
@@ -1161,7 +1185,7 @@ long pipe_fcntl(struct file *file, unsigned int cmd, =
unsigned long arg)
...
+=09while (pipe_empty(pipe->head, pipe->tail)) {
 ...
 =09=09=09break;
@@ -1480,16 +1495,16 @@ static int opipe_prep(struct pipe_inode_info *pipe,=
 unsigned int flags)
 =09int ret;
=20
 =09/*
...
+=09=09=09i_tail++;
+=09=09=09ipipe->tail =3D i_tail;
 =09=09=09input_wakeup =3D true;
+=09=09=09o_len =3D obuf->len;
+=09=09=09o_head++;
+=09=09=09opipe->head =3D o_head;
 =09=09} else {
 =09=09=09/*
...
+=09=09=09pipe_buf_release(pipe, &pipe->bufs[p_head & p_mask]);
 =09=09}
+
+=09=09pipe->head =3D p_head;
 =09}
 }
=20
@@ -1011,18 +1040,20 @@ static void pipe_advance(struct iov_iter *i, size_t=
 size)
...
+=09=09i->head =3D i_head;
 ...
 =09}
 =09i->count -=3D size;
@@ -1053,25 +1084,27 @@ void iov_iter_revert(struct iov_iter *i, size_t unr=
oll)
 =09i->count +=3D unroll;
...
+=09=09i->head =3D i_head;
 ...
 =09=09return;
 =09}
@@ -1159,13 +1192,13 @@ void iov_iter_pipe(struct iov_iter *i, unsigned int=
 direction,
...
@@ -1323,9 +1357,8 @@ static ssize_t pipe_get_pages_alloc(struct iov_iter *=
i,
 =09=09   size_t *start)
 {
 =09struct page **p;
...
=20
 =09if (!maxsize)
 =09=09return 0;
@@ -1333,9 +1366,9 @@ static ssize_t pipe_get_pages_alloc(struct iov_iter *=
i,
 =09if (!sanity(i))
 =09=09return -EFAULT;
=20
...
+=09npages =3D pipe_space_for_user(iter_head, i->pipe->tail, i->pipe);
 ...
 =09if (maxsize > n)
 =09=09maxsize =3D n;
@@ -1344,7 +1377,7 @@ static ssize_t pipe_get_pages_alloc(struct iov_iter *=
i,
...
+=09n =3D __pipe_get_pages(i, maxsize, p, iter_head, start);
 ...
 =09=09*pages =3D p;
 =09else
@@ -1560,15 +1593,15 @@ int iov_iter_npages(const struct iov_iter *i, int m=
axpages)
=20
 =09if (unlikely(iov_iter_is_pipe(i))) {
 =09=09struct pipe_inode_info *pipe =3D i->pipe;
